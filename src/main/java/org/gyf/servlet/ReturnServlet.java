 ...
 * stubID:所还桩号
 * 输出对象:字符串
 * 向客户端传送字符串含"success"表示还车成功否则失败
 * 还车操作需要对LocationInfo表，UserInfo表，BikeInfo表，StubInfo表同时更新保持一致性，一个出错恢复原状后返回失败
 *
 */
            ...
            if(userInfo == null)
                ...
                if(bikeID.equals("") || time == 0 )
                    ...
                    if(stubInfo == null)
                        ...
                        if(!stubInfo.getBike().equals(""))
                            msg = "The stub has a bike!";
                        else {
                            String bike = userInfo.getBike();
                            MysqlDao bikeDao = new BikeMysqlDaoImpl();
                            BikeInfo bikeInfo = (BikeInfo) bikeDao.Query(bike);
                            if (bikeInfo != null) {
                                if (!bikeInfo.getStub().equals("") || bikeInfo.getTime() == 0 || bikeInfo.getUser().equals(""))
                                    ...
                                    newStubInfo.setBike(bikeInfo.getId());
                                    ...
                                    newBikeInfo.setStub(stubInfo.getId());
                                    newBikeInfo.setTime(0);
                                    newBikeInfo.setUser("");
                                    ...
                                    newUserInfo.setTime(0);
                                    int newBalance = userInfo.getBalance() - 1 - (int) (currentTime - userInfo.getTime()) / 1000000;
                                    newUserInfo.setBalance(newBalance);
